...
[Sports-Reference](https://www.sports-reference.com/cbb/) — a deep historical
source hoopR did not previously cover (complementing KenPom, Bart Torvik and
CollegeBasketballData). It is the college arm of Sports Reference, so it reuses
hoopR's `bref_*` parser (comment-hidden tables; `data-stat`-keyed columns) and
needs no account or API key:

- **`srcbb_school_stats(season, table)`** — season school (team) statistics;
  `table = "basic"` / `"advanced"` / `"opponent"`.
- **`srcbb_rankings(season)`** — the season's AP poll, week by week.

Both echo the requested `season` (and `srcbb_school_stats()` the `table`) as
columns. Sports-Reference rate-limits aggressive scraping (~20 requests/minute);
