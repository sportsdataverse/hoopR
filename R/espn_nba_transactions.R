# espn_nba_transactions.R
# NBA-only ESPN endpoint wrappers:
#   espn_nba_draft(), espn_nba_freeagents(), espn_nba_transactions()
# These endpoints have no MBB equivalent, so they are implemented directly
...
#' }
espn_nba_draft <- function(season = most_recent_nba_season(), ...) {
  ...
  tryCatch(
    expr = {
      ...
      if (is.null(rounds) || length(rounds) == 0) {
        picks_df <- data.frame(stringsAsFactors = FALSE) %>%
          dplyr::as_tibble() %>%
          make_hoopR_data("ESPN NBA Draft Picks from ESPN.com", Sys.time())
        return(picks_df)
      }
    ...
    error = function(e) .report_api_error(
      e,
      hint = paste0("Failed to retrieve ESPN NBA draft data for season ", season),
      args = .args
    ),
    warning = function(w) .report_api_warning(
      w,
      hint = paste0("Warning retrieving ESPN NBA draft data for season ", season),
      args = .args
    ),
    finally = {}
  )
...
#' @name espn_nba_freeagents
NULL
#' @title
#' **Get ESPN NBA Free Agents**
#' @rdname espn_nba_freeagents
#' @author Saiem Gilani
#' @param season Season year (numeric, e.g. 2025). Defaults to the most
#'   recent NBA season.
#' @param ... Additional arguments; currently unused but retained for
...
#' }
espn_nba_freeagents <- function(season = most_recent_nba_season(), ...) {
  .args <- mget(setdiff(names(formals()), "..."))
  ...
  tryCatch(
    expr = {
      ...
      if (is.null(items) || (is.data.frame(items) && nrow(items) == 0) ||
        ...
        fa_df <- data.frame(stringsAsFactors = FALSE) %>%
          dplyr::as_tibble() %>%
          make_hoopR_data("ESPN NBA Free Agents from ESPN.com", Sys.time())
        return(fa_df)
      }
    ...
    error = function(e) .report_api_error(
      e,
      hint = "Failed to retrieve ESPN NBA free agents for season {season}",
      args = .args
    ),
    warning = function(w) .report_api_warning(
      w,
      hint = "Warning retrieving ESPN NBA free agents for season {season}",
      args = .args
    ),
    finally = {}
  )
...
#' @name espn_nba_transactions
NULL
#' @title
#' **Get ESPN NBA Transactions**
#' @rdname espn_nba_transactions
#' @author Saiem Gilani
#' @param season Season year (numeric, e.g. 2025). Defaults to the most
#'   recent NBA season.
...
#' }
espn_nba_transactions <- function(
  ...
  tryCatch(
    expr = {
      ...
      if (is.null(items) || (is.data.frame(items) && nrow(items) == 0) ||
        ...
        tx_df <- data.frame(stringsAsFactors = FALSE) %>%
          dplyr::as_tibble() %>%
          make_hoopR_data("ESPN NBA Transactions from ESPN.com", Sys.time())
        return(tx_df)
      }
    ...
    error = function(e) .report_api_error(
      e,
      hint = "Failed to retrieve ESPN NBA transactions for season {season}",
      args = .args
    ),
    warning = function(w) .report_api_warning(
      w,
      hint = "Warning retrieving ESPN NBA transactions for season {season}",
      args = .args
    ),
    finally = {}
  )
