...
#' @examples
#' \dontrun{
#'   get_box(browser, game_id = 1238, year = 2020)
#' }
...
get_box <- function(browser, game_id, year){
  ...
  refs <- (page %>%
             xml2::read_html() %>%
             rvest::html_nodes(xpath = "//*[@id='half-column3']//span//div[4]") %>%
             rvest::html_nodes("a"))[-1]

  ref_ranks <- page %>%
    xml2::read_html() %>%
    rvest::html_nodes(xpath = "//*[@id='half-column3']//span//div[4]") %>%
    rvest::html_nodes(".seed")
  ...
  for(i in 1:2){
    x <- (page %>%
            xml2::read_html() %>%
            rvest::html_nodes(".box-table"))[[i]]


    box_stat <- x %>%
      rvest::html_nodes(".box-stat")
...
#' @importFrom rlang .data
#' @importFrom assertthat assert_that
#' @importFrom rvest jump_to html_nodes html_table html_text
...
#' @examples
#' \dontrun{
#'   get_winprob(browser, game_id = 1238, year = 2020)
#' }

get_winprob <- function(browser, game_id, year){
  ...
  q <- (page %>%
          xml2::read_html() %>%
          rvest::html_nodes("#content-header") %>%
