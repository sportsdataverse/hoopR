...
nba_single_season <- function(season, p, dbConnection = NULL, tablename = NULL, qs = FALSE) {
  if (isTRUE(qs)) {
    ...
    .url <- glue::glue("https://github.com/saiemgilani/kenpomR-data/blob/master/nba/pbp/rds/play_by_play_{season}.qs")
    pbp <- qs_from_url(.url)
    
  }
  if (isFALSE(qs)) {
    .url <- glue::glue("https://raw.githubusercontent.com/saiemgilani/kenpomR-data/master/nba/pbp/rds/play_by_play_{season}.rds")
    con <- url(.url)
    pbp <- readRDS(con)
    close(con)
  }
...
load_nba_games <- function(){
  .url <- "https://raw.githubusercontent.com/saiemgilani/kenpomR-data/master/nba/nba_games_in_data_repo.csv"
  con <- url(.url)
...
#' @description `update_nba_db()` updates or creates a database with `kenpomR`
...
build_nba_db <- function(tblname = "kenpomR_nba_pbp", db_conn, rebuild = FALSE, show_message = TRUE) {
