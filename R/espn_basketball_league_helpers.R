...
# Internal helpers shared by MBB and NBA league-wide catalog wrappers.
...
.espn_bball_validate_league_cat <- function(league) {
  valid <- c("nba", "mens-college-basketball")
  if (!league %in% valid) {
    cli::cli_abort(
...
#' the MBB roster can exceed 10,000 entries across many pages.
#'
#' @param league character. `"nba"` or `"mens-college-basketball"`.
#' @param season numeric. Season year.
