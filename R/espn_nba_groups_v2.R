# espn_nba_groups_v2.R
# Public NBA shims for ESPN per-season group / conference endpoints.
# (Distinct from the existing site-v2 espn_nba_groups, which targets a
...
#' }
espn_nba_season_groups <- function(season = most_recent_nba_season(),
                                    season_type = c(2L, 3L), ...) {
  .espn_basketball_season_groups(league = "nba", season = season,
...
#' }
espn_nba_season_group <- function(group_id,
                                   season = most_recent_nba_season(),
                                   season_type = 2L, ...) {
  .espn_basketball_season_group(league = "nba", season = season,
                                  season_type = season_type,
                                  group_id = group_id, ...)
}
...
#' @title
#' **Get ESPN NBA Season Group Children Index**
#' @rdname espn_nba_season_group_children
#' @author Saiem Gilani
#' @description
...
#' }
espn_nba_season_group_children <- function(group_id,
                                            season = most_recent_nba_season(),
                                            season_type = 2L, ...) {
  .espn_basketball_season_group_children(league = "nba", season = season,
                                           season_type = season_type,
                                           group_id = group_id, ...)
}
...
#' @title
#' **Get ESPN NBA Season Group Teams Index**
#' @rdname espn_nba_season_group_teams
#' @author Saiem Gilani
#' @description
...
#' }
espn_nba_season_group_teams <- function(group_id,
                                         season = most_recent_nba_season(),
                                         season_type = 2L, ...) {
  .espn_basketball_season_group_teams(league = "nba", season = season,
                                        season_type = season_type,
                                        group_id = group_id, ...)
}
