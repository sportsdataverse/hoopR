...
| Function | Returns |
|---|---|
| `espn_mbb_athlete_info()` / `espn_nba_athlete_info()` | Bio and profile |
| `espn_mbb_athlete_overview()` / `espn_nba_athlete_overview()` | Season overview (web-common-v3) |
| `espn_mbb_athlete_stats()` / `espn_nba_athlete_stats()` | Season stats by category |
| `espn_mbb_athlete_gamelog()` / `espn_nba_athlete_gamelog()` | Game-by-game log |
| `espn_mbb_athlete_splits()` / `espn_nba_athlete_splits()` | Situational splits |
| `espn_mbb_athlete_eventlog()` / `espn_nba_athlete_eventlog()` | Event log (`$ref` links) |
| `espn_mbb_athlete_awards()` / `espn_nba_athlete_awards()` | Awards history |
| `espn_mbb_athlete_statisticslog()` / `espn_nba_athlete_statisticslog()` | Stats log (core-v2) |
...
| Function | Returns |
|---|---|
| `espn_mbb_event_odds()` / `espn_nba_event_odds()` | Opening / closing lines (NBA only -- empty for MBB) |
| `espn_mbb_event_probabilities()` / `espn_nba_event_probabilities()` | Win probability per play |
| `espn_mbb_event_officials()` / `espn_nba_event_officials()` | Officials |
...
`espn_mbb_event_odds()` always returns an empty tibble. The function
...
- **`espn_mbb_event_odds()`** -- always empty; ESPN doesn't carry NCAA
  lines.
