...
to *see* the momentum swing -- a win-probability line.

Two ways in, and the function names tell you which engine you're using.

```{r}
# Route A -- ESPN's per-play win probability.
wp <- espn_nba_event_probabilities(event_id = 401283399)

# Route B -- the NBA Stats API's own win-probability model.
# Note the prefix flips to `nba_` -- different data source, so the
# name's first token changes. Same idea, different kitchen.
wp_nba <- nba_winprobabilitypbp(game_id = "0022300001")
```

This is the grammar doing real work. `espn_nba_event_probabilities()`
and `nba_winprobabilitypbp()` answer nearly the same question, and you
can tell at a glance that the first is ESPN's take and the second is
NBA.com's, *because the prefix is the data source*. When two functions
look like near-synonyms, check the prefix -- that's the tiebreaker.
